...
import type {
  CurrentPlanDto as ICurrentPlanDto,
  DemandConditionDto as IDemandConditionDto,
...
export class CurrentPlanDto implements ICurrentPlanDto {
  ...
  @ApiProperty({
    required: false,
    example: 'KT망',
    description: '현재 이용 중인 통신망 (알뜰폰의 경우)',
  })
  @IsOptional()
  @IsString()
...
export class CreateSessionDto implements ICreateSessionDto {
  @ApiProperty({
    example: 'BOTH',
    description: '입력 타입 (PLAN, DEMAND, BOTH)',
  })
  @IsString()
