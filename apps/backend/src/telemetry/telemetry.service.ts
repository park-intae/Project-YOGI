...
export class TelemetryService {
  ...
  async handleDailyPipeline() {
    this.logger.log(
      '[Cron Pipeline] Daily Telemetry pipeline started at 04:00.',
    );
    try {
      ...
      this.logger.log(
        '[Cron Pipeline] Daily Telemetry pipeline completed successfully.',
      );
    } catch (error) {
      this.logger.error(
  ...
  async fetchData(targetCaseId: string): Promise<any> {
    ...
    if (!apiUrl) {
      this.logger.warn(
        '[Ingest] EXTERNAL_PLAN_API_URL is not set. Falling back to mock data.',
      );
      ...
      const selectedCase = mockData.mocks.find(
        (m: any) => m.case_id === targetCaseId,
      );
      ...
      if (selectedCase.status === 'ERROR') {
        this.logger.error(
          `[Ingest] Mock API Error occurred. HTTP Status: ${selectedCase.http_status}`,
        );
        throw new Error(
          `API_ERROR: ${selectedCase.response_data.message || 'Unknown error'}`,
        );
      }
    ...
    try {
      ...
      if (!root || !root.cmmMsgHeader || root.cmmMsgHeader.successYN !== 'Y') {
        throw new Error(
          `API Response Error or Invalid Format: ${JSON.stringify(root?.cmmMsgHeader || 'Unknown')}`,
        );
      }

      const rawPlans = root.alddlCharge || [];
  ...
  async ingest(mockCaseId?: string) {
    this.logger.log(
      `[Ingest] Starting ingestion. CaseID: ${mockCaseId || 'DEFAULT'}`,
    );

    const targetCaseId =
      mockCaseId || process.env.MOCK_CASE || 'MOCK_CASE_01_SUCCESS';
    ...
    this.logger.log(
      `[Ingest] Successfully ingested ${plans.length} raw plans into database.`,
    );
