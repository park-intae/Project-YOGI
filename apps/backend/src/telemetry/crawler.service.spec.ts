...
describe('CrawlerService', () => {
  ...
  describe('updatePlanUrls (성공 케이스)', () => {
    it('HTML에서 요금제 정보를 파싱하여 DB를 업데이트해야 한다 (단일 검증: 정상 파싱 및 매핑)', async () => {
      ...
      expect(prismaService.plan.update).toHaveBeenCalledWith({
        ...
        data: {
          planUrl:
            'https://www.epost.go.kr/comm.alddlord.RetrieveChargeDtl.comm?phonepaydivcd=N&bizcd=C0018&telecomcd=C03&charge_idn=99',
        },
      });
      ...
      expect(prismaService.plan.update).toHaveBeenCalledWith({
        ...
        data: {
          planUrl:
            'https://www.epost.go.kr/comm.alddlord.RetrieveChargeDtl.comm?phonepaydivcd=N&bizcd=C0019&telecomcd=S01&charge_idn=100',
        },
      });
    });
  });
  ...
  describe('updatePlanUrls (에러 케이스)', () => {
    it('외부 네트워크 통신 실패 시 에러를 던져야 한다 (단일 검증: 네트워크 에러)', async () => {
      ...
      // Act & Assert (실행 및 검증)
      await expect(crawlerService.updatePlanUrls()).rejects.toThrow(
        'Network timeout',
      );
    ...
    it('DB 업데이트 중 일부 실패가 발생하더라도 전체 프로세스는 완료되어야 한다 (단일 검증: DB 에러 복원력)', async () => {
      ...
      vi.spyOn(prismaService.plan, 'update').mockImplementation(
        async (args) => {
          if (args.where.id === 2) {
