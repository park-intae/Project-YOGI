...
describe('TelemetryService', () => {
  ...
  describe('ingest', () => {
    it('MOCK_CASE_01_SUCCESS가 주어지면 정상적으로 Plan 1차 덤프를 생성해야 한다', async () => {
      ...
      const fetchSpy = vi.spyOn(service as any, 'fetchData').mockResolvedValue({
        ...
        response_data: {
          plans: [
            { carrier: 'SKT', plan_name: 'TEST' },
            { carrier: 'KT', plan_name: 'TEST2' },
          ],
        },
      });
  ...
  describe('handleDailyPipeline', () => {
    ...
    it('ingest 단계에서 에러 발생 시 transform은 호출되지 않아야 한다', async () => {
      const ingestSpy = vi
        .spyOn(service, 'ingest')
        .mockRejectedValue(new Error('Ingest Error'));
  ...
  describe('fetchData', () => {
    it('should throw Error if external API fails', async () => {
      ...
      mockHttpService.get.mockReturnValueOnce(
        of({ data: '<invalid></invalid>' }),
      );

      await expect(service.fetchData('MOCK')).rejects.toThrow(
        'API Response Error',
      );
    ...
    it('should parse allowances containing string characters', async () => {
      ...
      const resultData = (service as any).parseDataAllowance(
        '매일5GB+소진시5Mbps',
      );
      expect(resultData).toBe(5);

      const resultVoice = (service as any).parseVoiceAllowance(
        '집/이동전화 무제한(영상/부가300분)',
      );
