...
import { BadRequestException, NotFoundException, ForbiddenException, ServiceUnavailableException } from '@nestjs/common';
...
describe('RecommendationsService', () => {
  ...
  describe('getRecommendationsData', () => {
    it('should throw NotFoundException if session not found', async () => {
      ...
      await expect(service.getRecommendationsData('invalid', 'session-123')).rejects.toThrow(NotFoundException);
    });

    it('should throw ForbiddenException if session ID mismatch', async () => {
      prisma.inputSession.findUnique = vi.fn().mockResolvedValue({ sessionId: 'other-session' });
      await expect(service.getRecommendationsData('input-123', 'session-123')).rejects.toThrow(ForbiddenException);
    });

    it('should return recommended plans data without AI', async () => {
      prisma.inputSession.findUnique = vi.fn().mockResolvedValue({ sessionId: 'session-123', userPlan: { baseFee: 60000 } });
      prisma.plan.findMany = vi.fn().mockResolvedValue([{ id: 'plan-1', planName: 'P1', baseFee: 40000 }]);

      const result = await service.getRecommendationsData('input-123', 'session-123');
  ...
  describe('getMoreRecommendationsData', () => {
    it('should throw NotFoundException if session not found', async () => {
      prisma.inputSession.findUnique = vi.fn().mockResolvedValue(null);
      await expect(service.getMoreRecommendationsData('invalid', 'session-123', [])).rejects.toThrow(NotFoundException);
    });

    it('should return recommended plans data excluding already given', async () => {
      prisma.inputSession.findUnique = vi.fn().mockResolvedValue({ sessionId: 'session-123' });
      prisma.plan.findMany = vi.fn().mockResolvedValue([{ id: 'plan-1' }, { id: 'plan-2', baseFee: 30000 }]);

      const result = await service.getMoreRecommendationsData('input-123', 'session-123', ['plan-1']);
      expect(result.recommended_plans.length).toBe(1);
  ...
  describe('getRecommendationSummary', () => {
    it('should throw NotFoundException if session not found', async () => {
      prisma.inputSession.findUnique = vi.fn().mockResolvedValue(null);
      await expect(service.getRecommendationSummary('invalid', 'session-123')).rejects.toThrow(NotFoundException);
    });
    ...
    it('should return mock summary in test mode', async () => {
      ...
      const result = await service.getRecommendationSummary('input-123', 'session-123');
      ...
      (service as any).genAI = {
        getGenerativeModel: vi.fn().mockReturnValue({
          generateContent: vi.fn().mockResolvedValue({
            response: {
              text: () => JSON.stringify({ ai_summary_comment: 'AI Summary Response' })
            }
          })
        })
      };
    ...
    it('should throw ServiceUnavailableException if Gemini API fails', async () => {
      ...
      await expect(service.getRecommendationSummary('input-123', 'session-123')).rejects.toThrowError(ServiceUnavailableException);
