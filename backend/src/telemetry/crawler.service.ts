...
export class CrawlerService {
  ...
  private readonly TARGET_URL = 'https://www.epost.go.kr/comm.alddl.RetrieveAlddlChargeList.comm';
   ...
   */
  async updatePlanUrls(): Promise<{ totalScraped: number; updated: number; failed: number }> {
    ...
    try {
        ...
        (i, el) => {
          ...
          if (match) {
            ...
            if (paramsString && paramsString[1]) {
              const params = paramsString[1].split(',').map((p) => p.replace(/['"]/g, '').trim());
              
              let bizcd, telecomcd, charge_idn;
              if (match[0].startsWith('goChoicePhoneCharge') && params.length >= 4) {
                 bizcd = params[1];
                 telecomcd = params[2];
                 charge_idn = params[3];
              } else if (match[0].startsWith('jsSelectChargeOrder') && params.length >= 3) {
                 bizcd = params[0];
                 telecomcd = params[1];
                 charge_idn = params[2];
              }
      ...
      this.logger.log(`Successfully scraped ${scrapedPlans.length} plans from the target site.`);
      ...
      for (const scraped of scrapedPlans) {
        ...
        if (matchedDbPlan) {
          try {
            ...
            this.logger.error(`Failed to update URL for plan ${matchedDbPlan.planName}`, e);
            failedCount++;
          }
        }
      }

      this.logger.log(`URL DB Sync Complete. Updated: ${updatedCount}, Failed: ${failedCount}`);
      
      return { totalScraped: scrapedPlans.length, updated: updatedCount, failed: failedCount };
    } catch (error) {
