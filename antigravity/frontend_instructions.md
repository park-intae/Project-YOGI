...
- 따라서 프론트엔드 AI 에이전트는 입력 폼 완료 시 백엔드가 응답하는 `input_id`를 동일한 페이지의 URL 쿼리 스트링(예: `/?input_id=xxxx`)에 즉시 바인딩해야 함.
- 결과 화면은 오직 이 `input_id` 하나만을 기반으로 단일 경로(`/`)에서 백엔드 API로부터 AI 추천 데이터(Top 3 결과 및 사유)를 다시 Fetching해 오도록 단방향 데이터 흐름을 강제할 것.

## 7. AI 파이프라인 지연(Latency) 방어 UX
- 사용자가 'AI 추천받기' 버튼을 클릭하면 백엔드 파이프라인(우체국 알뜰폰 데이터 검증 및 LLM Evaluation Loop)이 작동하므로 일반 CRUD API보다 응답 시간이 다소 길어질 수 있음.
