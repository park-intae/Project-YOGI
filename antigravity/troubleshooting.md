...
## 12. 통신망 필터링 로직에서 발생한 부분 문자열 매칭(Substring Matching) 버그
- **문제 발생**: 프론트엔드 요금제 추천 리스트에서 'KT망' 필터 버튼을 클릭했는데도 불구하고 SKT망 요금제들이 필터링되지 않고 화면에 섞여 나오는 현상이 발생함.
- **원인**: 
  - 통신망 필터링 로직에 자바스크립트의 `.includes()` 메서드를 사용하여 단순히 문자열 포함 여부만 검사했음.
  - 문제는 'SKT망'이라는 텍스트 안에도 'KT'라는 문자열이 포함되어 있어서(`'S' + 'KT' + '망'`), `'SKT망'.includes('KT')`가 `true`로 평가됨. 결과적으로 KT망 필터를 적용해도 SKT망 요금제까지 통과해 버리는 엣지 케이스 버그 발생.
- **해결 방안**: 
  - 단위 테스트(`RecommendationList.spec.tsx`) 작성 과정에서 해당 엣지 케이스를 테스트하던 중 발견함.
  - 필터링 로직을 수정하여, KT망 필터 선택 시 `baseNetwork.includes('KT') && !baseNetwork.includes('SKT')`와 같이 명시적으로 SKT가 포함된 경우를 배제하도록 조건을 엄격하게 개선하여 이슈 완벽 해결.